...
class CojFooterController extends GxController {
	...
	public function actionCreate() {
		$model = new CojFooter;
		$this->performAjaxValidation($model, 'coj-footer-form');
		if (isset($_POST['CojFooter'])) {
			$model->setAttributes($_POST['CojFooter']);
			if ($model->save()) {
	...
	public function actionUpdate($id) {
		$model = $this->loadModel($id, 'CojFooter');
		$this->performAjaxValidation($model, 'coj-footer-form');
		if (isset($_POST['CojFooter'])) {
			$model->setAttributes($_POST['CojFooter']);
			if ($model->save()) {
	...
	public function actionDelete($id) {
		if (Yii::app()->getRequest()->getIsPostRequest()) {
                    $this->loadModel($id, 'CojFooter')->delete();
                    if (!Yii::app()->getRequest()->getIsAjaxRequest()){
	...
	public function actionAdmin() {
		$model = new CojFooter('search');
		$model->unsetAttributes();
		if (isset($_GET['CojFooter'])){
			$model->setAttributes($_GET['CojFooter']);
                }
