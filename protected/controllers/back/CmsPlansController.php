...
class CmsPlansController extends GxController {
	...
	public function actionCreate() {
		$model = new CmsPlans;
		$this->performAjaxValidation($model, 'cms-plans-form');
		if (isset($_POST['CmsPlans'])) {
			$model->setAttributes($_POST['CmsPlans']);
			if ($model->save()) {
	...
	public function actionUpdate($id) {
		$model = $this->loadModel($id, 'CmsPlans');
		$this->performAjaxValidation($model, 'cms-plans-form');
		if (isset($_POST['CmsPlans'])) {
			$model->setAttributes($_POST['CmsPlans']);
			if ($model->save()) {
	...
	public function actionDelete($id) {
		if (Yii::app()->getRequest()->getIsPostRequest()) {
                    $this->loadModel($id, 'CmsPlans')->delete();
                    if (!Yii::app()->getRequest()->getIsAjaxRequest()){
	...
	public function actionAdmin() {
		$model = new CmsPlans('search');
		$model->unsetAttributes();
		if (isset($_GET['CmsPlans'])){
			$model->setAttributes($_GET['CmsPlans']);
                }
