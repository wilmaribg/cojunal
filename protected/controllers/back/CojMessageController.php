...
class CojMessageController extends GxController {
	...
	public function actionCreate() {
		...
		if (isset($_POST['CojMessage'])) {
			$model->setAttributes($_POST['CojMessage']);
			if ($model->save()) {
	...
	public function actionUpdate($id) {
		$model = $this->loadModel($id, 'CojMessage');
		if (isset($_POST['CojMessage'])) {
			$model->setAttributes($_POST['CojMessage']);
			if ($model->save()) {
	...
	public function actionDelete($id) {
		if (Yii::app()->getRequest()->getIsPostRequest()) {
                    $this->loadModel($id, 'CojMessage')->delete();
                    if (!Yii::app()->getRequest()->getIsAjaxRequest()){
	...
	public function actionAdmin() {
		$model = new CojMessage('search');
		$model->unsetAttributes();
		if (isset($_GET['CojMessage'])){
			$model->setAttributes($_GET['CojMessage']);
                }
