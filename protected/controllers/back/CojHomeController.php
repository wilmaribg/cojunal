...
class CojHomeController extends GxController {
	...
	public function actionCreate() {
		$model = new CojHome;
		$this->performAjaxValidation($model, 'coj-home-form');
		if (isset($_POST['CojHome'])) {
			$model->setAttributes($_POST['CojHome']);
			if ($model->save()) {
	...
	public function actionUpdate($id) {
		$model = $this->loadModel($id, 'CojHome');
		$this->performAjaxValidation($model, 'coj-home-form');
		if (isset($_POST['CojHome'])) {
			$model->setAttributes($_POST['CojHome']);
			if ($model->save()) {
	...
	public function actionDelete($id) {
		if (Yii::app()->getRequest()->getIsPostRequest()) {
                    $this->loadModel($id, 'CojHome')->delete();
                    if (!Yii::app()->getRequest()->getIsAjaxRequest()){
	...
	public function actionAdmin() {
		$model = new CojHome('search');
		$model->unsetAttributes();
		if (isset($_GET['CojHome'])){
			$model->setAttributes($_GET['CojHome']);
                }
