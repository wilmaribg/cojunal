...
class TypesController extends GxController {
	...
	public function actionCreate() {
		$model = new Types;
		$this->performAjaxValidation($model, 'types-form');
		if (isset($_POST['Types'])) {
			$model->setAttributes($_POST['Types']);
			if ($model->save()) {
	...
	public function actionDelete($id) {
		if (Yii::app()->getRequest()->getIsPostRequest()) {
                    $this->loadModel($id, 'Types')->delete();
                    if (!Yii::app()->getRequest()->getIsAjaxRequest()){
	...
	public function actionAdmin() {
		...
		if (isset($_GET['Types'])){
			$model->setAttributes($_GET['Types']);
                }
