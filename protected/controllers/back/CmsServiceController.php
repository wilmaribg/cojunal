...
class CmsServiceController extends GxController {
	...
	public function actionCreate() {
		...
		$this->performAjaxValidation($model, 'cms-service-form');
		if (isset($_POST['CmsService'])) {
			$model->setAttributes($_POST['CmsService']);
			if ($model->save()) {
	...
	public function actionUpdate($id) {
		$model = $this->loadModel($id, 'CmsService');
		$this->performAjaxValidation($model, 'cms-service-form');
		if (isset($_POST['CmsService'])) {
			$model->setAttributes($_POST['CmsService']);
			if ($model->save()) {
	...
	public function actionDelete($id) {
		if (Yii::app()->getRequest()->getIsPostRequest()) {
                    $this->loadModel($id, 'CmsService')->delete();
                    if (!Yii::app()->getRequest()->getIsAjaxRequest()){
	...
	public function actionAdmin() {
		$model = new CmsService('search');
		$model->unsetAttributes();
		if (isset($_GET['CmsService'])){
			$model->setAttributes($_GET['CmsService']);
                }
