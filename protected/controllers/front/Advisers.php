...
class Advisers extends AdvisersBase
{
	...
	public static function getCoordinators()
	{
		$sql = "SELECT idAdviser, name, email, active,
			...
			(SELECT COUNT(id) FROM wallets_by_coordinator WHERE idCoordinator = a.idAdviser) AS num_camapanas
			FROM advisers a
	...
	public static function getAssesoresPerfil($id=0)
	{
		...
		$sql = "SELECT 
				...
				advisers
				WHERE
				advisers.status_idStatus = $id AND
				advisers.active = 0";
	...
	//  {
	// 	 $sql = "SELECT advisers.name,
	// 	 		advisers.status_idStatus,
	// 	 		advisers.idAdviser
	//    			FROM advisers
	// 			   WHERE advisers.status_idStatus = 2";
