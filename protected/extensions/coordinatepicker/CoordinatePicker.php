...
class CoordinatePicker extends CWidget
{
    ...
    public function run()
    {
        ...
        $cs->registerScriptFile($this->assets . '/jquery.coordinate_picker'.(Yii::app()->language == "es" ? "" : "_en").'.js');
        $cs->registerCssFile($this->assets . '/styles/smodal/shadow.css');

        if (!isset($this->label)) $this->label = 'Pick Coordinates';

        echo CHtml::link($this->label, '#', array('id' => $this->getId(), 'class'=>'btn btn-yellow btn-small'));
