...
class GiixModelCode extends ModelCode {
			...
			if ($column->isForeignKey) {
				...
				if ($label === 'Id')
					$label = 'ID';
                                ...
                                if($column->comment != ""){
                                   $data = array_column($this->parseComment($column->comment), 'name');
                                   if(isset($data[0])){
                                       $label = $data[0];
                                   }
                                }
	...
	public function generateRules($table) {
			...
			if (!(!$column->allowNull && $column->defaultValue === null))
				$null[] = $column->name;
                        ...
                        if(isset($data[0])){
                            switch ($data[0]){
                                case 'image':
                                    $rulesAvance['images'][] = $column->name;
                                    break;
                                case 'file':
                                    $rulesAvance['files'][] = $column->name;
                                    break;
                                case 'email':
                                    $rulesAvance['emails'][] = $column->name;
                                    break;
                                case 'url':
                                    $rulesAvance['urls'][] = $column->name;
                                    break;
                                case 'date':
                                    $rulesAvance['dates'][] = $column->name;
                                    break;
                                case 'datetime':
                                    $rulesAvance['datetimes'][] = $column->name;
                                    break;
                                case 'time':
                                    $rulesAvance['times'][] = $column->name;
                                    break;
                            }
                        }
		}
		if ($null !== array()){
                ...
                if(isset($rulesAvance['images'])){
                    $rules[] = "array('" . implode(', ', $rulesAvance['images']) . "', 'match', 'pattern'=>'/\.(gif|jpg|jpeg|png|tiff)$/i','message'=> Yii::t('app', 'El {attribute} es inválido, Solo los archivos con estas extensiones son permitidos: gif, jpg, jpeg, png, tiff'), 'allowEmpty'=>true)";
                }
                if(isset($rulesAvance['files'])){
                    $rules[] = "array('" . implode(', ', $rulesAvance['files']) . "', 'match', 'pattern'=>'/\.(xls|xlsx|doc|docx|pdf)$/i','message'=> Yii::t('app', 'El {attribute} es inválido, Solo los archivos con estas extensiones son permitidos: xls, xlsx, doc, docx, pdf'), 'allowEmpty'=>true)";
                }
                if(isset($rulesAvance['emails'])){
                    $rules[] = "array('" . implode(', ', $rulesAvance['emails']) . "', 'email')";
                }
                if(isset($rulesAvance['urls'])){
                    $rules[] = "array('" . implode(', ', $rulesAvance['urls']) . "', 'url')";
                }
                if(isset($rulesAvance['dates'])){
                    $rules[] = "array('" . implode(', ', $rulesAvance['dates']) . "', 'date', 'format'=>'yyyy-M-d')";
                }
                if(isset($rulesAvance['datetimes'])){
                    $rules[] = "array('" . implode(', ', $rulesAvance['datetimes']) . "', 'date', 'format'=>'yyyy-M-d H:m:s')";
                }
                if(isset($rulesAvance['times'])){
                    $rules[] = "array('" . implode(', ', $rulesAvance['times']) . "', 'date', 'format'=>'H:m:s')";
                }
