...
?>
	...
	<?php echo $form->errorSummary($model, '<button type="button" class="close" data-dismiss="alert">&times;</button>', null, array('class'=>'alert alert-danger')); ?>
        <div class="row">
            ...
            <?php echo $form->textField($model, 'actionName', array('class'=>'form-control input-block-level')); ?>
            <?php echo $form->error($model,'actionName'); ?>
            </div>
        </div>
		<label><?php echo GxHtml::encode($model->getRelationLabel('effects')); ?></label>
		<?php echo $form->checkBoxList($model, 'effects', GxHtml::encodeEx(GxHtml::listDataEx(Effects::model()->findAllAttributes(null, true)), false, true)); ?>
