...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('wallets-has-campaigns-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<?php $this->widget('booster.widgets.TbGridView', array(
        ...
        //'rowCssClassExpression'=>'"items[]_{$data->idWalletsHasCampaigns}"',
        ...
        'columns' => array(
            array(
                'name'=>'idWallet',
                'value'=> 'Wallets::model()->findByPk($data->idWallet)->legalName',
                //'filter'=>GxHtml::listDataEx(Wallets::model()->findAllAttributes(null, true)),
                ),
			array(
                'name'=>'idCampaign',
                'value'=> 'Campaigns::model()->findByPk($data->idCampaign)->name',
                //'filter'=>GxHtml::listDataEx(Wallets::model()->findAllAttributes(null, true)),
                ),
		array(
