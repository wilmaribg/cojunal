...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('actions-has-effects-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<?php $this->widget('booster.widgets.TbGridView', array(
        ...
        //'rowCssClassExpression'=>'"items[]_{$data->idActionsHasEffects}"',
        ...
        'columns' => array(
			array(
                'name'=>'idAction',
                'value'=>'Action::model()->findByAttributes(array("idAction"=>$data->idAction))->actionName',
                                'filter'=>GxHtml::listDataEx(Action::model()->findAllAttributes(null, true)),
                ),  
            array(
                'name'=>'idEffect',
                'value'=>'Effects::model()->findByAttributes(array("idEffect"=>$data->idEffect))->effectName',
                                'filter'=>GxHtml::listDataEx(Effects::model()->findAllAttributes(null, true)),
                ),
   //          'idAction',
			// 'idEffect',
		array(
